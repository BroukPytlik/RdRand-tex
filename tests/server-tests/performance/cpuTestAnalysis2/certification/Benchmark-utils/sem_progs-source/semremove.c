/*
 * It will set the semaphore value to 0 and destroy it
 * ./semremove sem_id number_of_pids_waiting_for_this_semaphore
 * gcc -Wall -O2 -o semremove semremove.c
 * SEM_ID=$(semcreate) 
...
int main(int argc, char *argv[]) {
	...
	wait_for_all_pids(sem_id, pid_count, 5);

	/* set semval to 0 */
	if (semctl(sem_id, 0, SETVAL, sem_union) == -1) {
		perror("  semctl SETVAL to 0");
		semctl (sem_id, 0, IPC_RMID, sem_union);
		exit(1);
	}

	/* Make sure that nobody is waiting for this semaphore anymore. Sleep for (N-1)*N/2 seconds,  n=1=>0 sec*/	    
	wait_for_all_pids(sem_id, 0, 1);
