...
double test_throughput( int threads, const size_t chunk, int stop_after, FILE *stream, const int type)
{
	...
	int key,i;
	unsigned int retry;
	clock_t thread_times[threads];
	clock_t total_time;
	uintmax_t total_runs;
	...
	do
	{
		...
		for ( i=0; i<threads; ++i)
		{
			...
			//fprintf(stderr, "thread %u, clocks: %ju\n",i, (uintmax_t)(clock() - clk));

			thread_times[i] += clock() - clk;
		}
		...
		//fprintf(stderr, "total clocks: %ju\n",(uintmax_t)(clock() - clk_total));
		total_time += clock() - clk_total;
	...
	fprintf(stderr,"total clocks (avg): %ju\n", (uintmax_t)total_time/total_runs);
	for(i=0; i<threads; i++){
		fprintf(stderr,"thread %u clocks (avg): %ju\n", i, (uintmax_t)thread_times[i]/total_runs);
	}
