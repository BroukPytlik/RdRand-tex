...
\par{
The statistical tests passed successfully with tens of TB of generated data. This can't provide reliable information whether the RdRand has a backdoor or some side-channel security hole (see \fullref{sec:security} for more details), but it shows that the generated values have good statistical properties.
}
...
\par{
On the \fullref{fig:testing:threadsScalability} can be seen that the average performance per one thread is about 170 MiB/s up to four threads. Then, on about 730 MiB/s is the performance peak, where it is not possible to get higher speed anymore by adding more threads. Because the performance gets stable now (does not drop), this stabilisation cannot be caused by scheduler (as the scheduler cause the drop down later in the graph). Thus this has to be the performance limit of the Intel Secure Key. 
}
